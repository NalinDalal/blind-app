 ...
 */
export function getStudentDetailsFromEmail(email: string): StudentDetails | null {
    // Updated Regex to capture the semester and the full roll number separately
    const emailRegex = /^(\d{4})([a-zA-Z]+)(\d{2})(\d{1})([a-zA-Z0-9]+)@oriental\.ac\.in$/;
    ...
    const studentDetails: StudentDetails = {
        ...
        admissionType: rollNumber.toUpperCase().startsWith('D') ? 'Diploma (Lateral Entry)' : 'Regular',
    };
...
const newRegularEmail = generateStudentEmail(
    'Oriental Institute of Science and Technology', 'CS', 2023, 1, 253, 'Regular'
);
const newDiplomaEmail = generateStudentEmail(
    'Oriental Institute of Science and Technology', 'CS', 2024, 3, 2, 'Diploma (Lateral Entry)'
);

console.log(`Generated Regular Email should be 0105cs231253@oriental.ac.in ->`, newRegularEmail);
console.log(`Generated Diploma Email should be 0105cs243D02@oriental.ac.in ->`, newDiplomaEmail);
