...
/**
 * Handle POST requests to create a comment for a post.
 *
 * Validates the request JSON body for `content`, `postId`, and `authorId` (optional `parentId` for replies),
 * performs a toxicity check (logging a moderation event and rejecting toxic content), verifies the post and user exist,
 * creates the comment, conditionally creates a notification for the post author when the commenter is different,
 * and increments the post's engagement score by 5.
 *
 * @param req - NextRequest whose JSON body must include `content`, `postId`, and `authorId`; may include `parentId` to indicate a reply
 * @returns The created comment object on success; a JSON error response on failure describing the cause of the failure
 */
