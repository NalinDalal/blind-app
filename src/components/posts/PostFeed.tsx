...
const PostFeed = () => {
    ...
    if (status === "error") {
        return (
            <div
                className="flex flex-col items-center justify-center p-10 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-lg">
    ...
    return (
        ...
        <div>
            ...
            >
                {data.pages.map((page) => (
                    <React.Fragment key={Math.ceil(new Date().getMilliseconds() * Math.random() * 100)}>
                        {page.posts.map((post) => (
                            <PostItem
                                key={post.id}
                                post={post}
